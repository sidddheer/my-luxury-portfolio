...
const ProjectDetail = ({ project, onClose }: { project: Project, onClose: () => void }) => {
    ...
    return (
        ...
        >
            ...
            {/* CONTENT */}
            {/* CHANGED: Optimized padding (p-4) for mobile */}
            <div className="max-w-6xl mx-auto w-full p-4 md:p-12 pb-24 flex flex-col gap-12 md:gap-16 relative z-10">
                
                {/* HERO HEADER */}
                <div className="border-l-2 border-white/20 pl-6 md:pl-8 relative">
                    <motion.div 
                        initial={{ height: 0 }} 
                        animate={{ height: "100%" }} 
                        className={`absolute left-[-2px] top-0 w-[2px] ${bgColor}`}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-8 flex flex-col gap-12">
                        ...
                        <section className="group">
                            ...
                            <p className="text-zinc-300 leading-relaxed text-base md:text-lg border border-white/5 bg-white/5 p-6 rounded-sm">
                                {project.problem}
                            </p>
                        </section>
                        ...
                        <section>
                            ...
                            <div className="space-y-4">
                                {project.methodology.map((item, i) => (
                                    <div key={i} className="flex gap-4">
                                        <div className={`mt-2 min-w-[6px] h-[6px] rounded-full ${bgColor}`}></div>
                                        <p className="text-zinc-400 leading-relaxed text-sm md:text-base">{item}</p>
                                    </div>
                                ))}
                            </div>
                        </section>
  ...
  return (
    ...
    >
      ...
      <section className="relative z-10 h-screen flex flex-col justify-center px-6 md:px-12 pointer-events-none">
        <motion.div style={{ y: heroTextY, opacity: heroOpacity }} className="max-w-[90vw]">
          <div className="text-xs font-mono text-zinc-400 mb-6 tracking-widest uppercase">Data Analyst & Architect</div>
          {/* CHANGED: text-6xl for mobile, 12vw for desktop. Adjusted leading. */}
          <h1 className="text-6xl md:text-[12vw] leading-[0.9] md:leading-[0.8] font-bold tracking-tighter text-black">
            RAW DATA <br /><span className="text-zinc-300">REFINED</span> <br />INSIGHT.
          </h1>
        </motion.div>
        <motion.div style={{ opacity: heroOpacity }} className="absolute bottom-12 right-12 flex gap-4 items-center text-xs font-mono text-zinc-400">
      ...
      <div className="relative z-20 w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <div className="bg-black text-white min-h-screen p-8 md:p-20 flex flex-col gap-12 shadow-[-50px_0_100px_rgba(0,0,0,0.1)] mt-[10vh]">
                ...
                {/* --- NEW: MOBILE ONLY STATUS BAR --- */}
                <div className="lg:hidden flex items-center gap-2 pt-2 border-b border-zinc-800 pb-6 mb-2">
                    <span className={`w-2 h-2 rounded-full ${status === 'scanning' ? 'bg-red-500 animate-ping' : 'bg-green-500 animate-pulse'}`}></span>
                    <span className="text-xs font-mono text-zinc-500 uppercase tracking-widest">
                        System: {status === 'idle' ? 'Active' : 'WARNING'}
                    </span>
                </div>
      ...
      {status === 'breached' && (
            ...
            >
                ...
                >
                    {/* Intro */}
                    <div className="min-w-[80vw] md:min-w-[40vw] pr-20 snap-center shrink-0">
                        <h2 className="text-[12vw] md:text-[10vw] leading-none font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-600">DEEP <br/> STORAGE</h2>
                        <div className="mt-8 font-mono text-cyan-400 text-xs tracking-widest border-l-2 border-cyan-400 pl-4 flex flex-col gap-1">
                            <span className="font-bold"><ScrambleText text="WARNING: ENCRYPTION BROKEN" active={true} /></span>
                            <span className="text-cyan-400/60">ACCESS LEVEL: UNRESTRICTED</span>
                        </div>
                    ...
                    {PROJECT_DATA.map((proj) => (
                         <div 
                            key={proj.id}
                            onClick={() => setSelectedProject(proj)}
                            // CHANGED: Reduced internal padding from p-12 to p-6 on mobile
                            className="min-w-[85vw] md:min-w-[45vw] h-[60vh] bg-zinc-950/80 backdrop-blur-md border border-white/20 p-6 md:p-12 flex flex-col justify-between hover:bg-zinc-900/90 hover:border-white/50 transition-all duration-500 mr-12 snap-center rounded-sm group relative overflow-hidden cursor-pointer"
                        >
                            <div className={`absolute top-0 right-0 w-32 h-32 ${proj.color.replace('text-', 'bg-')}/10 rounded-full blur-3xl group-hover:${proj.color.replace('text-', 'bg-')}/20 transition-all`}></div>
                            <div className="flex justify-between items-start relative z-10">
                                <div className="p-3 bg-white/5 border border-white/10 rounded-full group-hover:scale-110 transition-transform">
                            ...
                            <div className="relative z-10">
                                <h3 className={`text-3xl md:text-5xl font-bold mb-6 mt-12 text-white group-hover:${proj.color.replace('text-', 'text-').replace('400', '100')} transition-colors`}>
                                    {proj.title}
                                </h3>
                                <p className="text-zinc-400 font-mono text-sm leading-relaxed max-w-md border-l border-zinc-700 pl-4 line-clamp-3">
                                    {proj.problem}
