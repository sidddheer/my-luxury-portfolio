...
def train_hybrid_model(crime_df, social_df):
  ...
  { 
    ...
    description: "A predictive analytics engine for construction sites that anticipates safety incidents by correlating quantitative labor data (fatigue) with qualitative superintendent notes (sentiment).",
    problem: `Construction projects suffer from "Death by a Thousand Cuts"—small, unreported frustrations that accumulate into major safety incidents. Traditional management relies on lagging indicators (Budget vs. Actual), which only reveal problems after they happen. Furthermore, valuable qualitative data—daily Superintendent logs—is often treated as "flat text" and ignored in risk modeling, despite containing early warning signs of crew confusion or morale decay.`,
    solution: `I engineered "Site Pulse," a predictive risk engine that digitizes site instinct. The system integrates structured labor data (hours, crew size) with unstructured field notes. I used NLTK/VADER to perform sentiment analysis on the notes, converting subjective entries (e.g., "confusion on drawings") into quantitative risk features. These sentiment scores were combined with fatigue metrics (Rolling 7-Day Overtime) to train a Random Forest Classifier. The model outputs a daily "Risk Probability" for each crew, visualized on a Power BI Heatmap to enable pre-emptive safety interventions.`,
    stack: ["Python", "NLTK (VADER)", "Random Forest", "Power BI", "Pandas"],
    metric: "Risk Precision: 85%",
    progress: 95,
    color: "text-orange-600",
...
const ProjectDetail = ({ project, onClose }: { project: ProjectDef, onClose: () => void }) => {
    ...
    return (
        <motion.div 
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            className="fixed inset-0 z-[2000] bg-zinc-950/60 backdrop-blur-sm flex items-center justify-center md:p-8"
        >
            <motion.div 
                initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: 'spring', damping: 25, stiffness: 200 }}
                className="w-full max-w-6xl h-[100vh] md:h-[90vh] bg-[#0c0c0c] md:border border-white/10 md:rounded-xl shadow-2xl flex flex-col overflow-hidden relative"
            >
                {/* 1. TITLE BAR (Sticky) */}
                <div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-white/5 bg-[#0a0a0a] shrink-0 z-10 sticky top-0">
                    <div className="flex items-center gap-3 md:gap-4 overflow-hidden min-w-0">
                        ...
                        <div className="hidden md:flex font-mono text-xs text-zinc-400 items-center gap-2 overflow-hidden whitespace-nowrap">
                            <Database className="w-3 h-3 shrink-0" />
                            <span>projects</span>
                            <span className="text-zinc-600">/</span>
                            <span>{project.category.toLowerCase().replace(' ', '_')}</span>
                            <span className="text-zinc-600">/</span>
                            <span className="text-white truncate">{project.id}_manifest.json</span>
                        </div>

                        {/* Mobile Short Title (Visible only on Mobile) */}
                        <div className="md:hidden font-mono text-xs text-white truncate flex items-center gap-2 min-w-0">
                             <FileJson className="w-3 h-3 text-cyan-500 shrink-0" />
                             <span className="truncate">{project.id}_manifest.json</span>
                        </div>
                    </div>

                    {/* Desktop Close Button (Always visible now) */}
                    <button onClick={onClose} className="text-zinc-500 hover:text-white transition-colors shrink-0 p-2 ml-2 bg-zinc-900/50 rounded-full md:bg-transparent">
                        <X className="w-5 h-5 md:w-6 md:h-6" />
                    </button>
                </div>

                {/* 2. SCROLLABLE CONTENT */}
                <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col lg:flex-row pb-24 md:pb-0">
                    
                    {/* LEFT: NARRATIVE */}
                    <div className="flex-1 p-6 md:p-12 space-y-10 md:space-y-12">
                        
                        {/* Header */}
                        <div>
                            <h1 className="text-3xl md:text-5xl font-bold text-white mb-4 tracking-tight">{project.title}</h1>
                            <p className="text-base md:text-lg text-zinc-400 font-light leading-relaxed border-l-2 border-white/10 pl-4">{project.description}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-1 gap-8">
                             <div className="space-y-3">
                                ...
                                <p className="text-sm md:text-base text-zinc-300 leading-relaxed">{project.problem}</p>
                             </div>
                             ...
                             <div className="space-y-3">
                                ...
                                <p className="text-sm md:text-base text-zinc-300 leading-relaxed">{project.solution}</p>
                             </div>
                        </div>
                        ...
                        {project.codeSnippet && (
                            <div className="space-y-3">
                                ...
                                <div className="bg-[#050505] border border-white/10 rounded p-4 md:p-6 overflow-x-auto">
                                    <pre className="font-mono text-[10px] md:text-xs text-zinc-300 leading-relaxed">{project.codeSnippet}</pre>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    {/* RIGHT: SIDEBAR (METADATA & STATS) */}
                    <div className="w-full lg:w-[350px] bg-[#0a0a0a] border-t lg:border-t-0 lg:border-l border-white/5 p-8 flex flex-col gap-10">
                ...
                {/* 3. MOBILE FLOATING CLOSE BUTTON (FIXED) */}
                <motion.button 
                    initial={{ scale: 0 }} 
                    animate={{ scale: 1 }} 
                    transition={{ delay: 0.5 }}
                    onClick={onClose}
                    className="md:hidden fixed bottom-8 left-1/2 -translate-x-1/2 bg-white text-black px-6 py-3 rounded-full font-bold shadow-2xl flex items-center gap-2 z-[2002]"
                >
