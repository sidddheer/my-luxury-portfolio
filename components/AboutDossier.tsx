...
// A Reusable Holographic Panel with Corner Brackets
// UPDATED: Added responsive padding (p-4 for mobile, p-6 for desktop)
const HoloPanel = ({ children, className = "", delay = 0 }: { children: React.ReactNode, className?: string, delay?: number }) => (
  <motion.div 
    initial={{ opacity: 0, scale: 0.9, borderColor: 'rgba(6,182,212,0)' }}
    animate={{ opacity: 1, scale: 1, borderColor: 'rgba(6,182,212,0.3)' }}
    transition={{ duration: 0.5, delay }}
    className={`relative bg-black/60 backdrop-blur-md border border-cyan-500/30 p-4 md:p-6 ${className}`}
  >
    {/* Corner Brackets */}
    <div className="absolute top-0 left-0 w-4 h-4 border-l-2 border-t-2 border-cyan-400"></div>
    <div className="absolute top-0 right-0 w-4 h-4 border-r-2 border-t-2 border-cyan-400"></div>
...
const DataRing = () => (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-20">
        <div className="w-[300px] h-[300px] md:w-[500px] md:h-[500px] border border-cyan-500/30 rounded-full animate-[spin_10s_linear_infinite]"></div>
        <div className="absolute w-[250px] h-[250px] md:w-[450px] md:h-[450px] border border-dashed border-cyan-500/30 rounded-full animate-[spin_15s_linear_infinite_reverse]"></div>
        <div className="absolute w-[200px] h-[200px] md:w-[300px] md:h-[300px] border-2 border-t-transparent border-b-transparent border-cyan-500/20 rounded-full animate-spin"></div>
    </div>
);
...
export default function AboutPage() {
  return (
    <main className="min-h-screen bg-black text-cyan-50 font-mono selection:bg-cyan-500/30 overflow-x-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 pt-24 md:pt-32 pb-24 max-w-7xl">
        
        {/* --- HEADER: IDENTITY VERIFICATION --- */}
        {/* CHANGED: items-start for mobile, items-end for desktop */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b-2 border-cyan-900/50 pb-6 mb-12 relative gap-8 md:gap-0">
            <div className="absolute bottom-0 right-0 w-32 h-2 bg-cyan-500/50"></div>
            ...
            >
                ...
                {/* CHANGED: text-4xl for mobile */}
                <h1 className="text-4xl md:text-7xl font-bold tracking-tighter text-white uppercase" style={{ textShadow: "0 0 20px rgba(6,182,212,0.5)" }}>
                    SID<span className="text-cyan-500">.DATA</span>
                </h1>
                <div className="flex flex-wrap gap-2 md:gap-4 text-xs md:text-sm text-cyan-200/70 uppercase tracking-widest">
                    <span>// Data Scientist</span>
                    <span>// Financial Analyst</span>
                    <span>// Business Intel</span>
                </div>
            ...
            <motion.div 
                initial={{ opacity: 0 }} 
                animate={{ opacity: 1 }} 
                className="w-full md:w-auto flex flex-col items-start md:items-end gap-6"
            >
                {/* LOCATION BLOCK */}
                <div className="text-left md:text-right space-y-1">
                    <div className="flex items-center justify-start md:justify-end gap-2 text-cyan-400">
                        <MapPin size={16} />
                        <span className="font-bold">SECTOR: IRVING_TX</span>
                    </div>
                    <div className="text-[10px] text-zinc-500">COORDS: 32.8140° N, 96.9489° W</div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                    ...
                    {/* LINKEDIN BUTTON */}
                    <Link 
                        href="https://www.linkedin.com/in/siddhartha-dheer-860437247/" 
                        target="_blank" 
                        className="flex-1 md:flex-none justify-center group flex items-center gap-3 px-5 py-2 rounded-full border border-zinc-700 bg-zinc-900/50 hover:border-cyan-500 hover:bg-cyan-500/10 hover:shadow-[0_0_15px_rgba(6,182,212,0.4)] transition-all duration-300"
                    >
                        <Linkedin className="w-4 h-4 text-zinc-400 group-hover:text-cyan-400 transition-colors" />
                        <span className="text-xs font-bold text-zinc-400 group-hover:text-cyan-400 tracking-widest transition-colors">LINKEDIN</span>
                    </Link>
                    ...
                    {/* GITHUB BUTTON */}
                    <Link 
                        href="https://github.com/sidddheer" 
                        target="_blank"
                        className="flex-1 md:flex-none justify-center group flex items-center gap-3 px-5 py-2 rounded-full border border-zinc-700 bg-zinc-900/50 hover:border-cyan-500 hover:bg-cyan-500/10 hover:shadow-[0_0_15px_rgba(6,182,212,0.4)] transition-all duration-300"
                    >
                        <Github className="w-4 h-4 text-zinc-400 group-hover:text-cyan-400 transition-colors" />
                ...
                <HoloPanel className="flex flex-col md:flex-row items-center justify-between gap-6 bg-gradient-to-r from-zinc-900 to-black hover:border-green-500/50 transition-colors group" delay={0.9}>
                    ...
                    <Link href="/contact" className="w-full md:w-auto">
                        <motion.button 
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="w-full md:w-auto px-8 py-4 bg-white text-black font-bold rounded-full hover:bg-cyan-50 transition-colors shadow-[0_0_20px_rgba(255,255,255,0.3)] flex items-center justify-center gap-2"
                        >
                            <Zap className="w-4 h-4 text-black fill-black" />
                            START A PROJECT
                        </motion.button>
        ...
        {/* --- FOOTER: SYSTEM LOG --- */}
        <div className="mt-12 border-t border-cyan-900/50 pt-6 flex flex-col md:flex-row justify-between items-center opacity-60 gap-2 md:gap-0">
            <div className="flex gap-4 font-mono text-[10px] text-cyan-700">
                <span>MEM_USAGE: 42%</span>
                <span>LATENCY: 12ms</span>
                <span>ENCRYPTION: AES-256</span>
